...
(*       let flip = function (n, (s, sa, c)) -> (c, n) in *)
   ...
   let setup net_info nid prog =
      ignore(Printf.printf "Setting up node with id: %d\n" nid);

      (* List.iter (fun (v, _) -> Printf.printf "Found info for node with id: %d\n" v) net_info; *)
      ...
      let result = {
         ...
         ; nodes_cmsg = optmap (fun (nid', {ip; port}) ->
                              ignore(Printf.printf " Testing %d %d %d\n" nid' nid (port + List.length net_info));
                              ignore(Printf.printf "%s\n" ip);
                              if (nid <> -1 && nid' <> -1) || (nid = -1 && nid' = -1) then
                                 None
                              else                                 
                                 let sock = make_out_udp_socket ip (port + List.length net_info) in
                                 ignore(Printf.printf "Created socket creating %d\n" (port + List.length net_info));
                                 let chan = out_channel_of_descr sock in
                                 ignore(Printf.printf "Created channel creating %s\n" ip);
                                 ignore(Printf.printf "Finished creating %s\n" ip);
                                 Some (nid', (sock, chan)))
                           net_info
         ; st = init_method 0
         ; prog = prog
